...
% anomalous electromagnetic moments of the $\tau$ lepton
...
{\scriptsize \href{mailto:hamzeh.khanpour@cern.ch}{hamzeh.khanpour@cern.ch}}}
...
World-comparison plot for the anomalous tau magnetic moment \(\atau\),
%and electric dipole moment \(\dtau\), 
including LEP, heavy-ion, and CMS \(pp\) constraints [{\color{blue}\small  Rept. Prog. Phys. 87 (2024) 107801}]. } 
\end{figure}
\end{column}
...
\begin{itemize}
 \justifying
\small
\item The  $\gamma\gamma\to\tau^+\tau^-$ process probes the \(\gamma\tau\tau\) interaction directly and cleanly.
...
\item This makes the process attractive both for present measurements and for future-machine sensitivity studies (FCC, LHeC, LH$\mu$C, ...).
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\begin{figure}
...
\begin{itemize}
 \justifying
  \item The most general electromagnetic vertex of a charged lepton can be parameterized in terms of form factors:
\end{itemize}
\[
...
\vspace*{3.0cm}
\centering
\hspace*{0.4cm}\includegraphics[width=\linewidth]{Ctb.png}
\vspace{0.3em}
{\scriptsize
...
\end{itemize}
\end{block}
\begin{block}{SMEFTsim}
\scriptsize
\begin{itemize}
\item GitHub: \url{https://github.com/SMEFTsim/SMEFTsim}
\item Reference: \url{https://arxiv.org/abs/2012.11343}
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\small Present comparison: $\datau=0.008{\color{magenta}@}\Lam= {\color{red}2}~\mathrm{TeV}$ at truth level.
\end{exampleblock}
\end{frame}
%=====================================================
...
\begin{itemize}
\justifying
\item Physical weighted ratio: directly compares the differential spectra.
\item Legacy ratio plots: retain continuity with the earlier plotting style and support compact linear-fit summaries.
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\item The \(\mathrm{LH}\mu\mathrm{C}\) distribution is more central because its \(p\mu^+\) beam-energy configuration is less asymmetric than the \(pe\) setup of the LHeC. 
%As a result, the \(\gamma\gamma\) system has a smaller longitudinal boost, and the \(\tau^+\tau^-\) pair is produced closer to \(Y_{\tau^+\tau^-}\simeq 0\).
...
\item The benchmark anomalous coupling \(\datau=0.008\){\color{magenta}@}\(\Lam= {\color{red}2}~\mathrm{TeV}\) hardens the invariant-mass spectrum relative to the Standard Model.
\item The rapidity distributions do show a clear collider-dependent effect: the LH$\mu$C distribution is more central than the LHeC one.
...
\begin{itemize}
\justifying
\item Extend the study from truth level to {\color{blue}{realistic detector-level}} analyses.
\item Implement acceptance and reconstruction effects for the relevant visible $\tau$ final states.
\item Translate the present shape distortions into explicit projected constraints on \atau \, at the  {\color{red}LHeC} and {\color{blue}{LH$\mu$C}}.

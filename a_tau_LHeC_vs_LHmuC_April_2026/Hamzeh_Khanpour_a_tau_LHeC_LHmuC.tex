...
\tikzset{
    ...
    hatchblob/.style={circle,draw=black,pattern=north east lines,pattern color=black,minimum size=8mm,inner sep=0pt}
}

\title[Anomalous $\tau$ moments at the LHeC and LH$\mu$C]{Anomalous electromagnetic moments of the tau lepton\\at the LHeC and LH$\mu$C}
...
\small Compare the truth-level sensitivity of the \textbf{LHeC} and the \textbf{LH$\mu$C} to anomalous $\tau$ electromagnetic moments using a benchmark \(\datau=0.008\) in photon-induced $\tau$-pair production.
\end{block}
\end{frame}
%=====================================================
...
%=====================================================
\begin{frame}{Current experimental status of \atau}
\begin{columns}[T]
\begin{column}{0.57\textwidth}
\begin{itemize}
\justifying
\small
\item Earlier direct constraints came from \textbf{OPAL}, \textbf{L3}, and \textbf{DELPHI} in the LEP era.
\item Heavy-ion measurements by \textbf{ATLAS Pb+Pb} and \textbf{CMS Pb+Pb} extended the reach using photon-induced processes at low di-$\tau$ masses.
\item The recent \textbf{CMS pp} analysis observed $\gamma\gamma\to\tau^+\tau^-$ and set significantly improved limits on \atau\ and \dtau.
\item The present future-collider study is motivated by the same central idea: the BSM effect becomes more visible in the hard region of the mass spectrum.
\end{itemize}
...
\item The invariant-mass spectrum of the $\tau^+\tau^-$ pair is particularly powerful because anomalous dipole-type couplings harden the high-mass tail.
...
\begin{itemize}
 \justifying
    \item Event generation is performed with the \texttt{SMEFTsim\_top\_alphaScheme\_UFO} model.
    ...
    \item In the local banners, the BSM benchmark is realized using a nonzero tau dipole Wilson-coefficient entry and \(\Lam=2~\mathrm{TeV}\) for the EFT normalization.
    \item The study is designed as a \textbf{truth-level comparison} between the SM and a single illustrative BSM benchmark, not yet as a detector-level limit extraction.
\end{itemize}
\end{column}
...
\begin{tikzpicture}[scale=0.80]
  \node at (0,2.6) {\small SM};
  \draw[photon] (-1.7,0.1)--(-0.3,0.6);
  \draw[photon] (-1.7,2.3)--(-0.3,1.8);
  \fill (-0.2,0.65) circle (0.05);
  \fill (-0.2,1.75) circle (0.05);
  \draw[tautrack] (-0.2,0.65)--(-0.2,1.75);
  \draw[tautrack] (-0.2,0.65)--(1.5,0.1);
  \draw[tautrack] (-0.2,1.75)--(1.5,2.3);
  \node at (-1.95,0.0) {\scriptsize $\gamma$};
  \node at (-1.95,2.4) {\scriptsize $\gamma$};
  \node[TauGreen] at (1.9,0.0) {\scriptsize $\tau^+$};
  \node[TauGreen] at (1.9,2.4) {\scriptsize $\tau^-$};
\end{tikzpicture}
\end{column}
\begin{column}{0.32\textwidth}
\centering
\begin{tikzpicture}[scale=0.80]
  \node at (0,2.6) {\small linear BSM insertion};
  \draw[photon] (-1.7,0.1)--(-0.3,0.6);
  \draw[photon] (-1.7,2.3)--(-0.3,1.8);
  \fill (-0.2,0.65) circle (0.05);
  \node[blob] at (-0.2,1.75) {};
  \draw[tautrack] (-0.2,0.65)--(-0.2,1.75);
  \draw[tautrack] (-0.2,0.65)--(1.5,0.1);
  \draw[tautrack] (-0.2,1.75)--(1.5,2.3);
  \node at (-1.95,0.0) {\scriptsize $\gamma$};
...
\centering
\begin{tikzpicture}[scale=0.80]
  \node at (0,2.6) {\small quadratic BSM term};
  \draw[photon] (-1.7,0.1)--(-0.3,0.6);
  \draw[photon] (-1.7,2.3)--(-0.3,1.8);
  \node[blob] at (-0.2,0.65) {};
  \node[blob] at (-0.2,1.75) {};
  \draw[tautrack] (-0.2,0.65)--(-0.2,1.75);
  \draw[tautrack] (-0.2,0.65)--(1.5,0.1);
  \draw[tautrack] (-0.2,1.75)--(1.5,2.3);
...
\begin{block}{Interpretation of the generated samples}
 \justifying
\small The SM sample uses \texttt{NP=0}. The benchmark BSM sample uses \texttt{NP<=2}, so the resulting prediction contains the SM contribution together with interference and quadratic EFT effects in the chosen MadGraph operator-order convention.
\end{block}
\end{frame}
%=====================================================
...
\midrule
LHeC @ 1.2 TeV & 50.695 & 51.923 & $5\times10^6$ \\
LH$\mu$C @ 3.7 TeV & 55.094 & 56.461 & $5\times10^6$ \\
\bottomrule
\end{tabular}
...
\item These values are \textbf{Monte Carlo effective luminosities} used for the plot normalization logic; they are not the final projected machine luminosities for a full experimental analysis.
\end{itemize}
\end{frame}
%=====================================================
...
\section{LHeC vs LH$\mu$C Plot Comparison}
...
\justifying
\small
    \item In both colliders, the benchmark \(\datau=0.008\) produces a harder \(\Mtt\) spectrum than the SM prediction.
    \item The separation between the SM and BSM curves becomes more visible toward larger invariant mass.
    \item The effect is more pronounced for the \textbf{LH$\mu$C}, indicating stronger sensitivity to anomalous dipole effects in the hard tail.
\end{itemize}
\takehome{
 \justifying The invariant-mass tail already indicates that the LH$\mu$C is more promising than the LHeC for \atau\ sensitivity in the present truth-level setup.}
\end{frame}
%=====================================================
...
\begin{itemize}
 \justifying
    ...
    \item The rise is stronger for the \textbf{LH$\mu$C}, showing that the same benchmark \(\datau\) produces a larger relative deviation from the SM in that environment.
\end{itemize}
\takehome{
 \justifying
...
\begin{itemize}
 \justifying
    ...
    \item In the present benchmark study, the fitted slope is larger for the \textbf{LH$\mu$C} than for the \textbf{LHeC}.
...
\begin{itemize}
 \justifying
    ...
    \item The growth is stronger for the \textbf{LH$\mu$C} than for the \textbf{LHeC}.
\end{itemize}
\end{block}
\end{column}
...
\begin{exampleblock}{Scientifically careful conclusion}
 \justifying
\small Within the present \textbf{truth-level benchmark study} based on merged MadGraph LHE samples, the \textbf{LH$\mu$C} appears more promising than the \textbf{LHeC} for probing \atau\ through photon-induced \(\tau^+\tau^-\) production.
\end{exampleblock}
\end{column}
\end{columns}
\begin{itemize}
...
% \includegraphics[width=\linewidth]{cms_topology_gamma_gamma_tautau.png}
% TODO: add pp topology / exclusive vs dissociative screenshot
\placeholdergraphic{CMS topology / elastic, semiexclusive, and dissociative picture.}{0.54\textheight}
\end{column}
\end{columns}
\end{frame}
%=====================================================
...
\begin{itemize}
 \justifying
    \item We studied photon-induced \(\gamma\gamma\to\tau^+\tau^-\) production at two future lepton--hadron colliders: the \textbf{LHeC} and the \textbf{LH$\mu$C}.
    ...
    \item Across the invariant-mass, weighted-ratio, and fit-based comparisons, the \textbf{LH$\mu$C} exhibits the stronger high-mass response.
\end{itemize}
\begin{alertblock}{Bottom line}
 \justifying
\small In the present truth-level benchmark study, the \textbf{LH$\mu$C} is favored over the \textbf{LHeC} for probing the anomalous magnetic moment \atau\ in photon-induced $\tau$-pair production.
\end{alertblock}
\end{frame}
%=====================================================
...
\end{columns}
\vspace{0.5em}
\takehome{The weighted BSM/SM ratio remains the clearest visual summary of why the LHmuC appears more sensitive to \atau\ in this benchmark study.}
\end{frame}
%=====================================================

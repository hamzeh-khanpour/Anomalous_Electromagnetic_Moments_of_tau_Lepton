...
C         LP=SIGN(1,LPP(IB(2)))
          ...
          ENDIF
          A2=PDG2PDF(LPP(IB(2)),7, IB(2),XBK(IB(2)), QSCALE)
        ENDIF
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! a a > ta+ ta-
      PD(IPROC)=A1*A2
...
C     
      ...
      DOUBLE PRECISION A2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C             LP=SIGN(1,LPP(IB(1)))
     ...
     $         ,DSQRT(ALL_Q2FACT(1, IVEC)))
            ...
            IF (ABS(LPP(IB(2))).GE.1) THEN
C             LP=SIGN(1,LPP(IB(2)))
              A2(IVEC)=PDG2PDF(LPP(IB(2)),7, IB(2),ALL_XBK(IB(2),IVEC)
     $         ,DSQRT(ALL_Q2FACT(2, IVEC)))
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=A1(IVEC)*A2(IVEC)
